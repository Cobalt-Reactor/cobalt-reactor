...
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
/// A 2D position propagation mode
pub enum PositionPropagation {
    #[default]
    /// Position is relative to the parent
    Relative,
    /// Position is absolute (usually used for locking a child to a world position)
    Absolute,
}

impl Position2D {
    /// Create a new `Position2D`
    ...
    /// A Position representing a single unit in positive y
    pub const UP: Self = Self { x: 0., y: 1. };
    /// A Position representing a single unit in negative y
    pub const DOWN: Self = Self { x: 0., y: -1. };
    /// A Position representing a single unit in negative x
    pub const LEFT: Self = Self { x: -1., y: 0. };
    /// A Position representing a single unit in positive x
