...
#[cfg_attr(feature = "serde", derive(serde::Serialize, serde::Deserialize))]
/// A 2D rotation propagation mode
pub enum RotationPropagation {
    #[default]
    /// Rotate relative to the parent
    Relative,
    /// Rotation is absolute relative to parent (does not change rotation as the parent rotates)
    Absolute,
}

impl Rotation2D {
    /// Create a new `Rotation2D`
    pub fn new() -> Self {
        Default::default()
    }
    ...
    pub fn radians(&self) -> Radians {
        Radians::new(self.rot.as_radians())
    }
    ...
    /// Create a new `Rotation2D` from radians
    ...
    /// Create a new `Rotation2D` from degrees
