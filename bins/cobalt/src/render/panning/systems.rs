...
pub fn camera_zoom(
    ...
    for (cam, mut proj, mut pos) in &mut query {
        if cam.enabled {
            ...
            // If there is both a min and max boundary, that limits how far we can zoom. Make sure
                ...
                (mouse_normalized_screen_pos, cam.zoom_to_cursor)
            {
                ...
                // As we zoom out, we don't want the viewport to move beyond the provided boundary.
                // If the most recent change to the camera zoom would move cause
                // parts of the window beyond the boundary to be shown, we
                // need to change the camera position to keep the viewport within bounds. The four
                // if statements below provide this behavior for the min and max x
                // and y boundaries.
...
pub fn camera_movement(
    ...
    mut query: Query<(&PanCam, &mut Position2D, &OrthographicProjection)>,
    ...
    if let Ok(window) = primary_window.get_single() {
        ...
        for (cam, mut position, projection) in &mut query {
            if cam.enabled
                && cam
                    .grab_buttons
                    .iter()
                    .any(|btn| mouse_buttons.pressed(*btn) && !mouse_buttons.just_pressed(*btn))
            {
                ...
                let mut proposed_cam_transform = *position - delta_world;

                // Check whether the proposed camera movement would be within the provided
                // boundaries, override it if we need to do so to stay within
                // bounds.
                ...
                *position = proposed_cam_transform;
            }
        }
